...
- [x] 1. Clipboard utility and session accessor
  - [x] 1.1 Add `ClipboardError` to `exceptions.py`
    - Add a new exception class `ClipboardError(Exception)` with a docstring
    - _Requirements: 1.4, 4.4_

  - [x] 1.2 Create `src/agent_repl/clipboard.py`
  ...
  - [x] 1.V Verify task group 1
    - [x] All new tests pass: `uv run pytest -q tests/test_clipboard.py tests/test_session.py`
    - [x] All existing tests still pass: `uv run pytest -q tests/`
    - [x] No linter warnings introduced: `uv run ruff check src/ tests/`
    - [x] Requirements 1.1, 1.2, 1.4, 3.1, 3.2, 3.3, 4.1, 4.2, 4.3, 4.4 acceptance criteria met

<!-- SESSION BOUNDARY: Task group 1 is complete. Do NOT continue to task group 2 in this session. -->

- [x] 2. Checkpoint — Clipboard & Session Accessor Complete
  - All 242 tests pass. No linter issues in changed files.

<!-- SESSION BOUNDARY -->

- [x] 3. Slash command and keyboard shortcut
  - [x] 3.1 Add `/copy` command to `builtin_commands.py`
  ...
  - [x] 3.2 Add `Ctrl+Y` key binding to `TUIShell`
  ...
  - [x] 3.3 Write tests for `/copy` command and keyboard shortcut
  ...
  - [x] 3.V Verify task group 3
    - [x] All new tests pass: `uv run pytest -q tests/test_builtin_commands.py tests/test_tui.py`
    - [x] All existing tests still pass: `uv run pytest -q tests/`
    - [x] No linter warnings introduced: `uv run ruff check src/ tests/`
    - [x] Requirements 1.1–1.6, 2.1–2.5 acceptance criteria met

<!-- SESSION BOUNDARY: Task group 3 is complete. Do NOT continue to task group 4 in this session. -->

- [x] 4. Documentation and final verification
  - [x] 4.1 Update `README.md` with `/copy` command and `Ctrl+Y` shortcut
    - _Requirements: 1.5_

  - [x] 4.2 Update `docs/api.md` if it documents commands or key bindings
    - _Requirements: 1.5_

  - [x] 4.3 Final end-to-end verification
    - [x] All tests pass: `uv run pytest -q tests/` — 254 passed
    - [x] Linter clean: `uv run ruff check src/ tests/` — no new issues
